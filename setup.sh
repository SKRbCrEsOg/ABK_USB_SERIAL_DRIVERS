...
#
# The usbserial core is kept built-in (=y) while the common USB serial device
# drivers are enabled as modules (=m). The produced .ko names are exported via
# $GITHUB_ENV (ABK_EXTERNAL_MODULE_KO_LIST) for the ABK kernel-module packaging
# step. This module is designed for the after_patch stage and is safe to re-run.
...
# shellcheck disable=SC1091
source "$MODULE_DIR/scripts/usb_serial_drivers.sh"

abk_require_env KERNEL_ROOT DEFCONFIG CUSTOM_EXTERNAL_MODULE_STAGE

module_name="${ABK_MODULE_NAME:-USB Serial Drivers}"
...
case "$CUSTOM_EXTERNAL_MODULE_STAGE" in
  ...
  before_build)
    abk_log "before_build: nothing to do (drivers and configs are applied in after_patch)"
    ;;
